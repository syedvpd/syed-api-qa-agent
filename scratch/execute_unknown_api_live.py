...
schemas_count = len(spec_json.get('components', {}).get('schemas', {}))

print(f'Target API: JSONPlaceholder Live REST API ({TARGET_BASE})')
print(f'OpenAPI Version: {spec_json.get("openapi")}')
print(f'Spec URL: {SPEC_URL}')
print(f'SHA-256 Contract Hash: {contract_hash}')
print(f'Discovered Paths: {paths_count}')
print(f'Discovered Schemas: {schemas_count}')

print('\n=== 2. OBTAINING AUTH TOKEN & TRIGGERING NEW LIVE TEST RUN ===')
auth_token = None
for attempt in range(1, 10):
    try:
        print(f'Attempt {attempt}: Connecting to Render backend at {BASE_URL}/api/auth/token...')
        auth_req = urllib.request.Request(
            f'{BASE_URL}/api/auth/token',
            data=json.dumps({}).encode('utf-8'),
            headers={'Content-Type': 'application/json', 'User-Agent': 'Mozilla/5.0'}
        )
        with urllib.request.urlopen(auth_req, timeout=60, context=ctx) as auth_resp:
...
if not auth_token:
    raise RuntimeError('Failed to obtain auth token from Render after retries.')
...
with urllib.request.urlopen(req_run, timeout=30, context=ctx) as resp:
    run_data = json.loads(resp.read().decode('utf-8'))
    run_id = run_data.get('runId') or run_data.get('id')
    print(f'NEW Test Run Created on Render: ID = {run_id}')
...
with urllib.request.urlopen(summary_req, timeout=30, context=ctx) as resp:
...
print('[OK] ALL 4 PILLARS & INVARIANT CHECKS PASSED ON UNKNOWN LIVE API!')
...
with urllib.request.urlopen(evidence_req, timeout=30, context=ctx) as resp:
    ...
    if pass_sample:
        print('\n--- CANONICAL PASS EVIDENCE SAMPLE (LIVE UNKNOWN API) ---')
    ...
    if fail_sample:
        print('\n--- CANONICAL FAIL EVIDENCE SAMPLE (LIVE UNKNOWN API) ---')
    ...
    if blocked_sample:
        print('\n--- CANONICAL BLOCKED EVIDENCE SAMPLE (LIVE UNKNOWN API) ---')
...
with urllib.request.urlopen(pdf_req, timeout=30, context=ctx) as resp:
    pdf_bytes = resp.read()
...
with open('scratch/unknown_api_run_proof.json', 'w') as f:
    json.dump({
        'runId': run_id,
...
print('\nSaved canonical run proof to scratch/unknown_api_run_proof.json')
